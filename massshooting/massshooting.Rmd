...
dfull <- dfull %>% 
  mutate(
    ...
    totalOwners = as.numeric(`ownership rate`) * pop16 / 100) 
...
### Total incidents per state mapped with total number of gun owners
...
dfull %>% 
  ...
  ggplot(aes(state,total, col=totalOwners, size=totalOwners)) +
    ...
    ggtitle("Incidents per state mapped with total number of gun owners")

```

What we've mapped above is the total number of shootings per state with the number of total gunowners (calculated via ownership rate * population) colored in. We see a clearer trend between the number of gun owners and incidents here.

If we don't plot the states on the X-axis, we can get an even clearer picture of the relationship between the number of guns in a state and the number of incidents, below I plot the total number of shooting casualties.:

### Total casualties (wounded or killed) per state vs. total gun owners

```{r ownership6, echo=F}
#ownership6
temp <- dfull %>% 
  group_by(totalOwners, state) %>% 
  summarise(total=sum(killed+wounded)) 
temp %>% 
  ggplot(aes(totalOwners, total)) +
    geom_point() +
    geom_point(data=subset(temp, total>500), col='red', size=2) +
    geom_text(data=subset(temp, total>500),  label = subset(temp, total>500)$state, nudge_y=15) 
